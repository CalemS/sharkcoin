...
#if defined(NDEBUG)
# error "Sharkcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x7850f2e343adecae36da3e1648aa2fb864ada4afa00b4335bda3b77c79ee6ecc");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Sharkcoin: starting difficulty is 1 / 2^12
...
{
    // Sharkcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 10000 blocks, which will occur approximately every X years
    nSubsidy >>= (nHeight / 10000); // Sharkcoin:

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Sharkcoin: 1 days
static const int64 nTargetSpacing = 5 * 60; // Sharkcoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Sharkcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Sharkcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xf993fdcc0fcc553ea1118257cf779e981ddaaa8cba5c748635282cb754d42425");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "444571178971955220";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04e0e30061923d6d145faa5d57d43835686cc0295784882ee44509ca330e3e8a31a4dfe4dc3ac7c433c428fc490971a8d7500592522add87f8b490aab5db5ed5be") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1526064675;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084772626;

        if (fTestNet)
        {
            block.nTime    = 1526064646;
            block.nNonce   = 385293790;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x84acddf22ba4f6230f169ceece534a1713f98b05f155f3d4e298068763296ba1"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc1, 0xb5, 0xda }; // Sharkcoin:
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SharkcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SharkcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SharkcoinMiner(CWallet *pwallet)
{
    printf("SharkcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("sharkcoin-miner");
    ...
    try { loop {
        ...
        printf("Running SharkcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SharkcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SharkcoinMiner, pwallet));
}
